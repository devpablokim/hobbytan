...
import { useCohort, useTeams, useAllUsers, useCurriculum, useGoals } from '@/lib/super-wks/useFirestoreData';
...
// ─── Curriculum Editor ───
function CurriculumEditor({ teams, curriculum }: { teams: import('@/lib/super-wks/types').Team[]; curriculum: Curriculum[] }) {
    ...
    <div className="bg-[#111111] border border-[#262626] p-5 mb-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <select value={selectedTeam} onChange={e => setSelectedTeam(e.target.value)} className="text-xs border border-[#404040] bg-[#171717] text-neutral-400 px-3 py-1.5">
      ...
      {selectedTeam !== 'all' && (
        <div className="mb-3 px-3 py-2 border border-blue-500/20 bg-blue-500/5 text-xs text-blue-400">
          💡 {teams.find(t => t.teamId === selectedTeam)?.name} 전용 커리큘럼을 설정합니다.
        </div>
      )}
        ...
        {curriculum.map(week => (
          <div key={week.weekNumber} className="bg-[#0a0a0a] border border-[#1a1a1a] p-4 flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[10px] text-neutral-600 mt-1">과제 {week.assignments.length}건 · 자료 {week.materials.length}건</div>
            </div>
            <button onClick={() => startEdit(week)} className="px-3 py-1.5 text-xs text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/10 transition-colors shrink-0">✏️ 수정</button>
          </div>
        ))}
      </div>
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={`Week ${editingWeek} 커리큘럼 수정`}>
        <div className="space-y-4">
          <div><label className="block text-xs text-neutral-500 mb-1.5">제목</label><input value={editTitle} onChange={e => setEditTitle(e.target.value)} className="w-full border border-[#404040] bg-[#171717] text-white px-3 py-2 text-sm" /></div>
          <div><label className="block text-xs text-neutral-500 mb-1.5">설명</label><textarea value={editDesc} onChange={e => setEditDesc(e.target.value)} rows={3} className="w-full border border-[#404040] bg-[#171717] text-white px-3 py-2 text-sm resize-none" /></div>
...
// ─── Progress Controller ───
function ProgressController({ teams: teamsData }: { teams: import('@/lib/super-wks/types').Team[] }) {
              ...
              {weekKeys.map((w, i) => (
                <div key={w}>
                  ...
                  <div className="flex items-center gap-1">
                    <input type="range" min={0} max={100} value={progValues[team.teamId]?.[w] || 0} onChange={e => handleChange(team.teamId, w, Number(e.target.value))} className="flex-1 h-1 accent-emerald-500 bg-[#262626]" />
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-1">
          {tabs.map(tab => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`px-4 py-2 text-sm transition-colors ${activeTab === tab.key ? 'bg-emerald-500 text-white' : 'border border-[#404040] text-neutral-400 hover:text-white'}`}>
              {tab.label}
            </button>
          ))}
        </div>
      ...
      {activeTab === 'overview' && (
        <>
          ...
          <div className="bg-[#111111] border border-[#262626] p-5 mb-6">
            ...
            {allTeams.map(team => {
              const teamGoals = allGoals.filter(g => g.teamId === team.teamId);
          ...
          <div className="bg-[#111111] border border-[#262626] mb-6">
            <div className="flex items-center justify-between p-5 border-b border-[#262626]">
              <h2 className="font-semibold text-white">팀 관리 ({allTeams.length}팀)</h2>
                  ...
                  {allTeams.map(team => {
                    ...
                    return (
                      <tr key={team.teamId} className="border-b border-[#1a1a1a] hover:bg-[#1a1a1a]">
                        ...
                        <td className="px-4 py-3 text-sm text-neutral-400">{lead?.nickname || lead?.displayName || '-'}</td>
                        <td className="text-center px-4 py-3 text-sm text-neutral-400">{memberCount}명</td>
      ...
      {/* Tab: Curriculum */}
      {activeTab === 'curriculum' && <CurriculumEditor teams={allTeams} curriculum={allCurriculum} />}

      {/* Tab: Progress */}
      {activeTab === 'progress' && allTeams.length > 0 && <ProgressController teams={allTeams} />}
      ...
      <Modal isOpen={showTeamModal} onClose={() => setShowTeamModal(false)} title={editingTeam ? '팀 수정' : '팀 추가'}>
        <div className="space-y-4">
          ...
          <div><label className="block text-xs text-neutral-500 mb-1.5">팀 리더</label>
            <select className="w-full border border-[#404040] bg-[#171717] text-white px-3 py-2 text-sm"><option value="">선택</option>{allUsers.filter(u => u.role !== 'admin').map(u => (<option key={u.uid} value={u.uid}>{u.nickname || u.displayName}</option>))}</select>
          </div>
          <div className="flex gap-2 pt-2">
      ...
      <Modal isOpen={showStudentModal} onClose={() => setShowStudentModal(false)} title={editingStudent ? '수강생 수정' : '수강생 추가'}>
        <div className="space-y-4">
          ...
          <div><label className="block text-xs text-neutral-500 mb-1.5">팀 배치</label>
            <select className="w-full border border-[#404040] bg-[#171717] text-white px-3 py-2 text-sm"><option value="">미배치</option>{allTeams.map(t => <option key={t.teamId} value={t.teamId}>{t.name}</option>)}</select>
          </div>
      ...
      <Modal isOpen={showGoalModal} onClose={() => setShowGoalModal(false)} title="목표 추가">
        <div className="space-y-4">
          <div><label className="block text-xs text-neutral-500 mb-1.5">팀</label><select className="w-full border border-[#404040] bg-[#171717] text-white px-3 py-2 text-sm">{allTeams.map(t => <option key={t.teamId} value={t.teamId}>{t.name}</option>)}</select></div>
